...
pub trait SubDraw3 {
    ...
    fn draw_prof(&self) -> Result<Vec<u8>, String> {
        ...
        let lpv = vec![lp0, lp1, lp2, lp3];
        let yrs = vec!["2024", "2023", "2022", "2021"];
        let cols = vec![self.grn(), self.blu(), self.red(), self.gr3()];
        let mut lsy = vec![-1000f32, -1000f32, -1000f32, -1000f32];
        for i in 0..4 {
            let col = cols[i];
            let ii = (200 + i * 50) as i32;
            draw_text_mut(&mut image, col, ii, 12, sc2, &font, yrs[i]);
            for (di, v) in lpv[i].iter().enumerate() {
                let di = di % 48;
                ...
                if lsy[i] > -1000f32 {
                    draw_line_segment_mut(&mut image, (xi, yi), (xi, lsy[i]), col);
                }
                lsy[i] = yi;
